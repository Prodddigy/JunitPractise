...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		/** ERROR ENCOUNTERED
		 * // I found it by noticing that no new accounts were added to hashtable
		if (accountlist.containsKey(accountid)) {
			throw new AccountExistsException();
		}
		else {
			accountlist.get(accountid); <-- instead of .get(accountid) use .put(accountid,this.getCurrency()) to store the account
		}
		 */
		if (accountlist.containsKey(accountid)) {
			throw new AccountExistsException();
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		 ...
		 *
			if (accountlist.containsKey(accountid)) {
			throw new AccountDoesNotExistException();<-- this should be in else
		}
		else {
			Account account = accountlist.get(accountid);<-- these
			account.deposit(money);						<-- two should be right after the if condition above ^^^
		}
		 */
		if (accountlist.containsKey(accountid)) {
	 ...
	 * @throws AccountDoesNotExistException If the account does not exist
	 */
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {//error
		/** ERROR ENCOUNTERED
		 * instead of making a withdrawal in else a deposit is made instead
		 *
		 * 		if (!accountlist.containsKey(accountid)) {
		 * 			throw new AccountDoesNotExistException();
		 *                }
		 * 		else {
		 * 			Account account = accountlist.get(accountid);
		 * 			account.deposit(money); <-- here should be withdrawal
		 *        }
		 */
		if (!accountlist.containsKey(accountid)) {
			throw new AccountDoesNotExistException();
	 ...
	 * @throws AccountDoesNotExistException If the account does not exist
	 */
	public Integer getBalance(String accountid) throws AccountDoesNotExistException {//error
/**	ERROR ENCOUNTERED
 * 	if (!accountlist.containsKey(accountid)) {
 * 			throw new AccountDoesNotExistException();
 *                }
 * 		else {
 * 			return accountlist.get(accountid).getBalance().getAmount(); <-- unneeded usage of .getAmount()
 * 																getBalance() will suffice
 *        }
 */
	 ...
	 * @throws AccountDoesNotExistException If one of the accounts do not exist
	 */
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {//error
		/** ERROR ENCOUNTERED
		 * a simple mistake transfer is going from account A straight back to account A
